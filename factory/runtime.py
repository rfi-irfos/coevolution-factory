...
.foot{{color:#5b6675;font-size:12px;margin-top:36px}}
.cmoverlay{{position:fixed;inset:0;z-index:200;background:rgba(6,9,13,.72);backdrop-filter:blur(6px);padding:24px;overflow-y:auto}}
.cmpanel{{position:relative;margin:24px auto;width:100%;max-width:1180px;min-height:200px;height:min(860px,92vh);max-height:92vh;background:#0a0e14;border:1px solid #1c2733;border-radius:20px;box-shadow:0 30px 80px rgba(0,0,0,.6);overflow:hidden;display:flex;flex-direction:column;box-sizing:border-box}}
...
<div id=hcinfopop class=hcinfopop hidden><b>Was du hier siehst</b><div style="margin-top:5px;color:#c7d2e0">Jede Wabe ist eine eigenständige Firma. Klicke eine Wabe, um Details zu sehen.</div><div style="margin-top:6px;color:#8b98a9">Plus/Minus zum Zoomen, Ziehen zum Verschieben.</div></div>
</style>
<div class=zoomctl>
...
// Info bubble toggle
(function(){{
 var btn=document.getElementById('hcinfo');
 var pop=document.getElementById('hcinfopop');
 if(!btn||!pop)return;
